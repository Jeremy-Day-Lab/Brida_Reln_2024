...
mapply(FUN = function(x, z) {
  ...
  file_name_pca <- paste0("DESeq2_QC_LRT1/PCA_", z)
  ...
  file_name_heatmap <- paste0("DESeq2_QC_LRT1/sample_to_sample_heatmap_", z)
  # save output
...
# Make output folder
dir.create("DESeq2_DEGResults_LRT1")
...
dds_all_cells <- mapply(FUN = function(x, z) {
  ...
  # Dispersion plot
  file_name_dist <- paste0("DESeq2_DEGResults_LRT1/DispPlot_", z, ".pdf")
  pdf(file_name_dist)
  plotDispEsts(x)
  dev.off()
  return(x)
...
# Make output folder
dir.create("DESeq2_Reln_Collapsed_LRT1")

# Write out the files
mapply(FUN = function(x, z) {
  file_name <- paste0("DESeq2_Reln_Collapsed_LRT1/DESeq2_res_", z, ".csv")
  write.csv(x, file=file_name, row.names = TRUE, quote = FALSE)
}, x = res_all_cells, z = gsub("-", ".", names(res_all_cells)))

# Save counts
mapply(FUN = function(x, z) {
  file_name_norm <- paste0("DESeq2_Reln_Collapsed_LRT1/normalized_counts_", z, ".csv")
...
write.table(x         = DEGs_DF,
            file      = "DESeq2_Reln_Collapsed_LRT1/number_of_DEGs_Per_cellType.txt",
            sep       = "\t",
            col.names = TRUE,
            row.names = FALSE,
            quote     = FALSE)
