...
##        BETWEEN LACZ & RELN       ##
## CONTROLLING FOR FACS GROUP & SEX ##
##                                  ##
######################################
...
# Create column for grouping variable (we want to control for group & sex)
allRats$gp.sex <- as.factor(paste(allRats$group, allRats$sex, sep = "_"))
...
metadata$gp.sex <- as.factor(paste(metadata$group, metadata$sex, sep = "_"))
...
dds_all_cells <- mapply(FUN = function(x, z) {
  x <- DESeq(x, test="LRT", reduced = ~ gp.sex)
